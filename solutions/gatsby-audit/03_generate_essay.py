...
def generate_section(state: PipelineState) -> dict:
    ...
    context_block = "\n\n".join([
        f"[Chapter {p['chapter']}, Passage {p['chunk']}] (relevance: {p['similarity']:.3f}):\n{p['text']}"
        for p in passages
    ])

    system_prompt = """You are a literary scholar writing a formal academic analysis of 
F. Scott Fitzgerald's The Great Gatsby (1925). Write in an academic but engaging style. 
Every analytical claim must be supported by specific textual evidence from the provided passages.
Use proper MLA-style in-text citations referencing chapter numbers, e.g., (Fitzgerald, Ch. 1).
Write approximately 2-3 substantial paragraphs for this section."""

    user_prompt = f"""## Section: {section['title']}
## Metaphor/Motif: {section['metaphor']}
## Analysis Direction: {section['depth_strategy']}

## Source Passages from The Great Gatsby:
{context_block}

Write a detailed literary analysis of this metaphor/motif based on the passages above.
Ground every point in specific textual evidence. Include direct quotations where appropriate."""

    # 3. Generate with DeepSeek
    print(f"  Generating analysis...")
    start_time = time.time()
    content = call_deepseek(system_prompt, user_prompt, max_tokens=2048)
