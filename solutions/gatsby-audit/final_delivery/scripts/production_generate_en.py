...
def generate_section(state: PipelineState) -> dict:
    ...
    system_prompt = (
        "You are a Literary Analyst writing a 10-page master's level thesis on 'The Great Gatsby' (1925). "
        "You are mid-way through the document. Your tone is scholarly, precise, and sophisticated—suitable "
        "for a GS-15 level federal policy advisor.\n\n"
        "STRICT NEGATIVE CONSTRAINTS:\n"
        "1. DO NOT use the phrase 'In F. Scott Fitzgerald's The Great Gatsby' or any variation of it.\n"
        "2. DO NOT start with an introductory sentence about the book. Jump directly into the metaphor.\n"
        "3. DO NOT write a concluding paragraph or summary for this section. End with a sharp analytical point.\n"
        "4. DO NOT use duplicate headings. Only use the single ## heading provided.\n"
        "5. DO NOT use the word 'conclusion' or 'concluding' anywhere except in Section 11 (Synthesis).\n"
        "6. DO NOT repeat the metaphor name more than twice in the section.\n"
        "7. DO NOT include any meta-commentary, preamble, or notes about your thinking process.\n"
        "8. Output ONLY the polished analytical section, starting with the ## heading.\n\n"
        "POSITIVE STYLE DIRECTIVES:\n"
        "9. Write like Harold Bloom or Lionel Trilling — dense, allusive, every sentence carries argument.\n"
        "10. Every sentence must advance a claim. If a sentence could be deleted without losing an argument, delete it.\n"
        "11. Embed quotes mid-sentence: 'The voice, \"full of money\" (Ch.1), conflates desire with capital.' Never set quotes on their own line.\n"
        "12. Begin each paragraph with a bold claim, not a transition. First word should be a noun or the metaphor itself.\n"
        "13. Prefer: 'X reveals Y' over 'It can be argued that X serves to reveal Y.'\n"
        "14. Target: 420-450 words for this section. Dense, not padded.\n"
    )
    user_prompt = f"""Context:
{context_block}

Current Metaphor: ## {section['title']}

Task: Provide a deep-dive analysis of this metaphor grounded in the 1925 text.

Requirements:
1. Word Count: Strictly 425–475 words. This is critical for document pagination.
2. Style: Scholarly, GS-15 level prose. Use precise vocabulary (e.g., 'meretricious', 'stratification', 'industrial entropy').
3. Citations: Exactly two direct quotes from the provided context with Chapter citations (e.g., Chapter III).
4. Thematic Link: Connect this metaphor to the broader theme of {theme}.
5. Tone: Academic continuity. Write as if this is a middle section of a long chapter. No intros, no outros.
6. Reasoning: Before writing, use your internal thinking process to determine how this metaphor foreshadows the novel's tragic arc.

Output Format:
## {section['title']}
[Your Analysis Here — no other headings, no bold section labels]"""
...
def assemble_essay(state: PipelineState) -> dict:
    ...
    essay_parts.append("### An Examination of Fitzgerald's Symbolic Architecture and the Disintegration of the American Dream\n")

    for draft in sorted(state["drafts"], key=lambda d: d["section_number"]):
        # Section content already includes its own ## heading
    ...
    # Also save locally
    clean_essay = re.sub(r'<think>.*?</think>', '', full_essay, flags=re.DOTALL).strip()
    clean_essay = clean_essay.replace('\u201c', '"').replace('\u201d', '"')
    clean_essay = clean_essay.replace('\u2018', "'").replace('\u2019', "'")
    clean_essay = clean_essay.replace('\u2014', '—').replace('\u2013', '–')
